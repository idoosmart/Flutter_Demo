...
import com.idosmart.bluetooth.Config;
import com.idosmart.bluetooth.ble.config.BLEGattAttributes;
import com.idosmart.bluetooth.ble.config.UUIDConfig;
import com.idosmart.bluetooth.ble.callback.BluetoothCallback;
import com.idosmart.bluetooth.ble.callback.UserCloseBluetoothCallBack;
import com.idosmart.bluetooth.bt.HIDConnectManager;
import com.idosmart.bluetooth.bt.LePairConnectManager;
import com.idosmart.bluetooth.timer.ITimeOutPresenter;
import com.idosmart.bluetooth.timer.TimeOutPresenter;
import com.idosmart.bluetooth.utils.ByteDataConvertUtil;
import com.idosmart.bluetooth.utils.CommonUtils;
import com.idosmart.bluetooth.logger.Logger;
import com.idosmart.bluetooth.utils.PairedDeviceUtils;
import com.idosmart.bluetooth.utils.PhoneInfoUtil;
...
abstract class BaseConnect implements BluetoothCallback {
    ...
    private static int maxGattErrorRetryTimes = 2;//当前重试次数
                ...
                public void run() {
                    DeviceBusinessManager.checkBindState(gatt.getDevice().getAddress(), new Function1<Boolean, Unit>() {
                        ...
                        public Unit invoke(Boolean aBoolean) {
                            Logger.p("[BaseConnect:onConnectionStateChange()] BindState = " + aBoolean + " , current_reytime = " + current_reytime + " , time = " + (System.currentTimeMillis() - lastConnectTime));
                            //部分三星手机连接出现多次133策略，调试发现最慢第6次能连上#246886
                            if (status == 133 && PhoneInfoUtil.isSamsungDevice() && System.currentTimeMillis() - lastConnectTime < 10 * 1000) {
                                Logger.e("[BaseConnect:onConnectionStateChange()] samsung device , use 6 retry times");
                                maxGattErrorRetryTimes = 6;
                            } else {
                                maxGattErrorRetryTimes = 2;
                            }
                            if(!aBoolean && current_reytime < maxGattErrorRetryTimes){
...
//        }
        ...
        if (mState == STATE_CONNECTING ) {
            Logger.e("[BaseConnect] connect() is STATE_CONNECTING, state = " + mState);
            callOnConnecting();
            return;
        }
        if (mState == STATE_CONNECTED) {
            Logger.e("[BaseConnect] connect() is STATE_CONNECTED, state = " + mState);
            try {
