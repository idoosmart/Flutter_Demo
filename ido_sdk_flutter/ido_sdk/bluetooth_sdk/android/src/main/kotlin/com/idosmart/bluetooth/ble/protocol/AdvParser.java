package com.idosmart.bluetooth.ble.protocol;
...
public class AdvParser {
    ...
    public static List<String> decodeServiceUUIDs(byte[] data) {
        if (data == null) {
            return null;
        }
        ...
        try {
            ...
            for (int index = 0; index < packetLength; index++) {
                ...
                if (fieldName == SERVICE_CLASS_128BIT_UUID) {
                    //NodicDFUManager.log(TAG, "index: " + index + " Service class 128 bit UUID exist");
                    //Log.p(TAG, "index: " + index + " Service class 128 bit UUID exist");
                    String uuid = decode128BitServiceUUID(data, index + 1, fieldLength - 1);
                    if (!TextUtils.isEmpty(uuid)) {
                        serviceUUIDs.add(uuid);
                    }
                } else if (fieldName == SERVICE_CLASS_16BIT_UUID) {
                    String uuid = decode16BitServiceUUID(data, index + 1, fieldLength - 1);
                    if (!TextUtils.isEmpty(uuid)) {
                        serviceUUIDs.add(uuid);
                    }
                }
     ...
     * check for required DFU Service UUID = 0x1530 or in decimal 2148 inside 128 bit uuid
     */
    private static String decode128BitServiceUUID(byte[] data, int startPosition, int serviceDataLength) throws Exception {
        //NodicDFUManager.log(TAG, "StartPosition: " + startPosition + " Data length: " + serviceDataLength);
        Log.d(TAG, "StartPosition: " + startPosition + " Data length: " + serviceDataLength);
        if (data == null || data.length == 0) {
            return null;
     ...
     * check for required DFU Service UUID = 0x1530 or in decimal 2148 inside 128 bit uuid
     */
    private static String decode16BitServiceUUID(byte[] data, int startPosition, int serviceDataLength) throws Exception {
        //NodicDFUManager.log(TAG, "StartPosition: " + startPosition + " Data length: " + serviceDataLength);
        Log.d(TAG, "StartPosition: " + startPosition + " Data length: " + serviceDataLength);
        if (data == null || data.length == 0) {
            return null;
