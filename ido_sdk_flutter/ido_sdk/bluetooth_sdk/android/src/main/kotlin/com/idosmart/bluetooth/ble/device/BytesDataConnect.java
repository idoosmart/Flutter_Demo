...
import com.idosmart.bluetooth.ble.callback.DeviceGattCallBack;
import com.idosmart.bluetooth.utils.ByteDataConvertUtil;
import com.idosmart.bluetooth.utils.Constants;
...
//        }
    ...
    private void resendData(final byte[] data) {
        ...
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Logger.e("[BytesDataConnect] retry send => " + ByteDataConvertUtil.bytesToHexString(data, ByteDataConvertUtil.LIMITHEXLEN));
                mIsReSendData = false;
                mIsSendingCmdData = false;
                sendNextCmdData();
            }
    ...
    private boolean writeBytes() {
        ...
        Logger.e("[BytesDataConnect] sendata: "+ByteDataConvertUtil.bytesToHexString(currentByteDataRequest.getSendData(), ByteDataConvertUtil.LIMITHEXLEN));
...
//            }
    ...
    private void receiverDeviceData(byte[] values) {
       Logger.e("[BytesDataConnect] receive <= " + ByteDataConvertUtil.bytesToHexString(values, ByteDataConvertUtil.LIMITHEXLEN));
    }
    ...
    protected void addCmdData(ByteDataRequest request, boolean isForce) {
        ...
        Logger.e("[BytesDataConnect] addCmdData( " + ByteDataConvertUtil.bytesToHexString(request.getSendData(), ByteDataConvertUtil.LIMITHEXLEN) + "---+platform" +request.getPlatform());
    ...
    private void deviceResponseOnLastSend(byte[] values, int status) {
        if (status == BluetoothGatt.GATT_SUCCESS) {
            Logger.p("[BytesDataConnect] onDeviceResponseOnLastSend( " + ByteDataConvertUtil.bytesToHexString(currentByteDataRequest.getSendData(), ByteDataConvertUtil.LIMITHEXLEN) + ")");
        } else {
            Logger.e("[BytesDataConnect] onDeviceResponseOnLastSend[failed]( " + ByteDataConvertUtil.bytesToHexString(values, ByteDataConvertUtil.LIMITHEXLEN) + ")");
        }
