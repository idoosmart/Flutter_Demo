...
/// 协议状态
open class IDOProtocolState(
    internal var _isConnected: Boolean = false,
    internal var _isConnecting: Boolean = false,
    internal var _isBinding: Boolean = false,
    internal var _isFastSynchronizing: Boolean = false,
    internal var _otaType: IDOOtaType = IDOOtaType.NONE,
    internal var _macAddress: String = "UNKNOWN"
) {
    constructor() : this(false, false, false, false, IDOOtaType.NONE, "UNKNOWN")
    ...
    var isFastSynchronizing: Boolean
        get() = _isFastSynchronizing
        internal set(value) {
            _isFastSynchronizing = value
        }

    var otaType: IDOOtaType
        get() = _otaType
        internal set(value) {
            _otaType = value
        }
    ...
    var macAddress: String
        get() = if (isConnected) _macAddress else "UNKNOWN"
        internal set(value) {
            _macAddress = value
        }
    ...
    override fun toString(): String {
        return "_isConnected: $_isConnected, _isConnecting: $_isConnecting, _isBinding: $_isBinding, _isFastSynchronizing: $_isFastSynchronizing, _otaType: $_otaType, _macAddress: $_macAddress";
    }
}

/// 请求发送蓝牙数据
class IDOBleDataRequest {
    /// 响应数据
    ...
    constructor(data: ByteArray,type: Int,macAddress: String) {
        this.data = data
        this.type = type
...
class IDODeviceNotificationModel {
     ...
     * ```
     */
    var controlEvt: Int? = 0

    /** 控制事件返回值（部分事件才有） */
    var controlJson: String? = ""

    constructor(dataType: Int,notifyType: Int,msgId: Int,msgNotice: Int,
                errorIndex: Int,controlEvt: Int,controlJson:String) {
        this.dataType = dataType
        this.notifyType = notifyType
        this.msgId = msgId
        this.msgNotice = msgNotice
        this.errorIndex = errorIndex
        this.controlEvt = controlEvt
