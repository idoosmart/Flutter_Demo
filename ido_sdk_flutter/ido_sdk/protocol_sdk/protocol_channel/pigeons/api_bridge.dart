...
abstract class BridgeDelegate {
  ///  写数据到蓝牙设备
  void registerWriteDataToBle(BleData bleData);

  /// 监听状态通知（SDK)
  void listenStatusNotification(StatusNotification status);

  /// 监听设备主动通知/控制事件 (设备)
  void listenDeviceNotification(DeviceNotificationModel model);

  /// 协议库状态
  void listenProtocolStateChanged(ProtocolState protocolState);

  /// 根据设备macAddress查询绑定状态
  bool checkDeviceBindState(String macAddress);

  /// 发现处于OTA状态的设备(需要执行ota升级)
  void listenWaitingOtaDevice(OtaDeviceModel otaDevice);

  /// 算法原始数据采集 (内部使用)
  void listenDeviceRawDataReport(int dataType, String jsonStr);
}
...
abstract class Bridge {
  ...
  @async
  bool register(
      bool outputToConsole, bool outputToConsoleClib, bool isReleaseClib);
  ...
  @async
  bool markOtaMode(
      String macAddress, String iosUUID, int platform, int deviceId);
  ...
  /// ```
  @async
  bool markConnectedDevice(
      String uniqueId, int otaType, bool isBinded, String? deviceName);

  /// 标记设备已断开（蓝牙断开时调用）
  @async
  bool markDisconnectedDevice(String? macAddress, String? uuid);
