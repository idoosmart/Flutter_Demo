...
class _IDOExchangeData implements IDOExchangeData {
  ...
  late final _streamV2Exchange =
      StreamController<IDOV2ExchangeModel>.broadcast();
  late final _streamV3Exchange =
      StreamController<IDOV3ExchangeModel>.broadcast();
...
extension _IDOExchangeExt on _IDOExchangeData {
      ...
      if (code == 0) {
        ...
        if (evt == CmdEvtType.exchangeAppStartBlePause.evtType) {
          ...
          if (this.supportV3ActivityExchange) {
            _v3Model?.appStartBle2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          } else {
            _v2Model?.appStartBle2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          }
          ...
          data.model = model;
          if (this.supportV3ActivityExchange) {
            _v3Model?.appStartBleEnd2AppData(
                model.day ?? 0,
                model.hour ?? 0,
                model.minute ?? 0,
                model.second ?? 0,
            ...
            _v2Model?.appStartBleEnd2AppData(
                model.day ?? 0,
                model.hour ?? 0,
                model.minute ?? 0,
                model.second ?? 0,
        ...
        } else if (evt == CmdEvtType.exchangeAppBleStart.evtType) {
          // 蓝牙开始运动
          ...
          data.model = model;
          if (this.supportV3ActivityExchange) {
            _v3Model?.bleEnd2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          } else {
            _v2Model?.bleEnd2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          }
          ...
          data.model = model;
          if (this.supportV3ActivityExchange) {
            _v3Model?.blePause2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          } else {
            _v2Model?.blePause2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          }
          ...
          data.model = model;
          if (this.supportV3ActivityExchange) {
            _v3Model?.bleRestore2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          } else {
            _v2Model?.bleRestore2AppData(model.day ?? 0, model.hour ?? 0,
                model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
          }
          ...
          _v3Model?.blePlan2AppData(
              ...
              model.actionType ?? 0,
              model.errorCode ?? 0,
              model.trainingYear ?? 0,
              model.trainingMonth ?? 0,
              model.time ?? 0,
              model.lowHeart ?? 0,
              model.heightHeart ?? 0);
        }
  ...
  Future<bool> _getActivityHrData() {
    ...
    _libMgr.send(evt: CmdEvtType.exchangeAppGetV3HrData).listen((response) {
      ...
      logger?.d(
          'exchange get activity heart rate data error code == ${response.code}');
    ...
    _libMgr.send(evt: CmdEvtType.exchangeAppGetActivityData).listen((response) {
      ...
      if (_isSuccessCallback(response)) {
        ...
        _v3Model?.getActivityData(
            ...
            reply.warmUp ?? 0,
            reply.fatBurning ?? 0,
            reply.aerobicExercise ?? 0,
            reply.anaerobicExercise ?? 0,
            reply.extremeExercise ?? 0,
            reply.warmUpTime ?? 0,
            reply.fatBurningTime ?? 0,
            reply.aerobicExerciseTime ?? 0,
            reply.anaerobicExerciseTime ?? 0,
            reply.extremeExerciseTime ?? 0,
            reply.avgSpeed ?? 0,
            reply.avgStepStride ?? 0,
            reply.maxStepStride ?? 0,
            reply.kmSpeed ?? 0,
            reply.maxSpeed ?? 0,
            reply.fastKmSpeed ?? 0,
            reply.avgStepFrequency ?? 0,
            reply.maxStepFrequency ?? 0,
            reply.avgHrValue ?? 0,
            reply.maxHrValue ?? 0,
            reply.kmSpeedCount ?? 0,
            reply.actionDataCount ?? 0,
            reply.stepsFrequencyCount ?? 0,
            reply.miSpeedCount ?? 0,
            reply.recoverTime ?? 0,
            reply.vo2max ?? 0,
            reply.trainingEffect ?? 0,
            reply.grade ?? 0,
            reply.realSpeedCount ?? 0,
            reply.paceSpeedCount ?? 0,
            reply.inClassCalories ?? 0,
            reply.completionRate ?? 0,
            reply.hrCompletionRate ?? 0,
            reply.kmSpeeds ?? [],
            reply.stepsFrequency ?? [],
            reply.itemsMiSpeed ?? [],
            reply.itemRealSpeed ?? [],
            reply.paceSpeedItems ?? [],
            reply.actionData ?? [],
            reply.segmentItemNum ?? 0,
            reply.segmentTotalTime ?? 0,
            reply.segmentTotalDistance ?? 0,
            reply.segmentTotalPace ?? 0,
            reply.segmentTotalAvgHr ?? 0,
            reply.segmentTotalAvgStepFrequency ?? 0,
            reply.paceHiit ?? 0,
            reply.paceAnaerobic ?? 0,
            reply.paceLacticAcidThreshold ?? 0,
            reply.paceMarathon ?? 0,
            reply.paceEasyRun ?? 0,
            reply.segmentItems ?? [],
  ...
  Future<bool> _getActvityGpsData() {
    ...
    _libMgr.send(evt: CmdEvtType.getActivityExchangeGpsData).listen((response) {
      ...
      logger
          ?.d('exchange get activity gps data error code == ${response.code}');
  ...
  void _appExec(IDOBaseExchangeModel model) {
    ...
    if (model is IDOAppStartExchangeModel) {
      ...
      _libMgr
          .send(evt: CmdEvtType.exchangeAppStart, json: jsonStr)
          .listen((response) {
        ...
        logger?.d(
            'exchange app to ble start reply == ${response.json} error code == ${response.code}');
      ...
      _libMgr
        ...
        logger?.d(
            'exchange app to ble end reply == ${response.json} error code == ${response.code}');
      ...
      if (this.supportV3ActivityExchange) {
        _v3Model?.appRestore2BleData(model.day ?? 0, model.hour ?? 0,
            model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
        _streamV3Exchange.sink.add(_v3Model ?? IDOV3ExchangeModel());
      } else {
        _v2Model?.appRestore2BleData(model.day ?? 0, model.hour ?? 0,
            model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
        _streamV2Exchange.sink.add(_v2Model ?? IDOV2ExchangeModel());
      }
      //app发起运动恢复
      ...
      _libMgr
        ...
        logger?.d(
            'exchange app to ble restore reply == ${response.json} error code == ${response.code}');
      ...
      if (this.supportV3ActivityExchange) {
        _v3Model?.appPause2BleData(model.day ?? 0, model.hour ?? 0,
            model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
        _streamV3Exchange.sink.add(_v3Model ?? IDOV3ExchangeModel());
      } else {
        _v2Model?.appPause2BleData(model.day ?? 0, model.hour ?? 0,
            model.minute ?? 0, model.second ?? 0, model.sportType ?? 0);
        _streamV2Exchange.sink.add(_v2Model ?? IDOV2ExchangeModel());
      }
      //app发起运动暂停
      ...
      _libMgr
        ...
        logger?.d(
            'exchange app to ble pause reply == ${response.json} error code == ${response.code}');
      ...
      _libMgr
        ...
        logger?.d(
            'exchange app to ble v2 ing reply == ${response.json} error code == ${response.code}');
      ...
      _libMgr
        ...
        logger?.d(
            'exchange app to ble v3 ing reply == ${response.json} error code == ${response.code}');
        setStatus = ExchangeStatus.appIngReply;
        final data = ExchangeResponse();
        data.code = response.code;
        if (_isSuccessCallback(response)) {
          ...
          reply.sportType = _baseModel?.sportType;
          data.model = reply;
          _v3Model?.appIng2BleReplyData(
              reply.version ?? 0,
              reply.heartRate ?? 0,
              reply.distance ?? 0,
              reply.duration ?? 0,
              reply.realTimeCalories ?? 0,
              reply.realTimeSpeed ?? 0,
              reply.kmSpeed ?? 0,
              reply.steps ?? 0,
              reply.swimPosture ?? 0,
              reply.status ?? 0,
              reply.realTimeSpeedPace ?? 0,
              reply.trainingEffect ?? 0,
              reply.anaerobicTrainingEffect ?? 0,
              reply.actionType ?? 0,
              reply.countHour ?? 0,
              reply.countMinute ?? 0,
              reply.countSecond ?? 0);
      ...
      _libMgr
          .send(evt: CmdEvtType.exchangeAppPlan, json: jsonStr)
          .listen((response) {
        final data = ExchangeResponse();
        data.code = response.code;
        if (_isSuccessCallback(response)) {
          ...
          reply.sportType = _baseModel?.sportType;
          data.model = reply;
          _v3Model?.appPlan2BleReplyData(reply.operate ?? 0,
              reply.errorCode ?? 0, reply.planType ?? 0, reply.actionType ?? 0);
          _streamV3Exchange.sink.add(_v3Model ?? IDOV3ExchangeModel());
          if (_status == ExchangeStatus.appEndPlanReply) {
            /// 此处运动计划数据结束
          }
  ...
  void _appReplyExec(IDOBaseExchangeModel model) {
    ...
    if (model is IDOBleStartReplyExchangeModel) {
      ...
      _v3Model?.blePlan2AppReplyData(model.errorCode ?? 0, model.operate ?? 0,
          model.planType ?? 0, model.actionType ?? 0);
      _streamV3Exchange.sink.add(_v3Model ?? IDOV3ExchangeModel());
      if (model.operate == 4) {
        ///此处运动计划结束
      }
      ...
      _libMgr.send(
          evt: CmdEvtType.exchangeAppStartBlePauseReply, json: jsonStr);
      if (this.supportV3ActivityExchange) {
      ...
      _libMgr.send(
          evt: CmdEvtType.exchangeAppStartBleRestoreReply, json: jsonStr);
      if (this.supportV3ActivityExchange) {
        _v3Model?.appStartBle2AppReplyData(model.errCode ?? 0);
        _streamV3Exchange.sink.add(_v3Model ?? IDOV3ExchangeModel());
      } else {
      ...
      if (this.supportV3ActivityExchange) {
        _v3Model?.appStartBleEnd2AppReplyData(model.duration ?? 0,
            model.calories ?? 0, model.distance ?? 0, model.errCode ?? 0);
        _streamV3Exchange.sink.add(_v3Model ?? IDOV3ExchangeModel());
      } else {
        _v2Model?.appStartBleEnd2AppReplyData(model.duration ?? 0,
            model.calories ?? 0, model.distance ?? 0, model.errCode ?? 0);
        _streamV2Exchange.sink.add(_v2Model ?? IDOV2ExchangeModel());
      }
    }
