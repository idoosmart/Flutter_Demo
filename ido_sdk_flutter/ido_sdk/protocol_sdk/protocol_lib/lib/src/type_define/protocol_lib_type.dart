...
class IDODeviceNotificationModel {
  ...
  /// 84 固件通知APP SPP传输间隔,固件上报的spp传输间隔范围0-200   单位毫秒 ，放在parameter[5]字段中
  /// 85 手表音乐切换到手机音乐通知APP，APP收到通知会下发歌曲信息及播放状态，进度等
  /// 86 固件进入充电状态
  /// 87 固件退出充电状态
  /// 88 设备修改游戏密码配置后通知软件来获游戏取密码配置
  /// 89 APP发起测量功能的时候，固件通知APP手表測量失敗
  /// 90 APP发起测量功能的时候，固件通知APP手表测量成功，APP去获取测量值。
  /// 91 固件通知APP蓝牙断连后重连,设备(戒指等无屏设备)在运动中状态。以防app发起运动，杀死app后15秒内打开app重连，设备无法结束运动，需要app接收到通知停止设备运动。
  /// 92 手表和手机协议栈ble配对成功，手表通知APP
  /// 93 请保持佩戴10分钟以上再测量
  /// 94 女性生理日常记录修改，同步APP获取
  /// 95 固件用户信息有更改, 通知app获取(0209)                
  /// 96 固件左右手佩戴设置有更改, 通知app获取(0223)          
  /// 97 手表通知APP智能模式
  /// 106 设备可独立升级 设备升级开始
  /// 107 设备可独立升级 设备升级中
  /// 108 设备可独立升级 设备升级结束
  /// 109 固件通知app获取三诺血糖数据
  /// 110 固件通知app车锁数据更新，同步最新车锁数据
  /// 111 设备可独立升级 设备升级成功
  /// 112 设备可独立升级 设备升级失败
  /// 113 固件通知app来获取定位信息
  /// 114 小程序卸载，(设备可独立安装和卸载，用户卸载后需要通知APP获取列表)
  /// 115 设备可独立升级 设备下载OTA升级包失败
  /// 116 心率开关状态变更通知
  /// 117 手表通知APP获取血糖数据
  /// 118  手表通知APP更新习惯养成奖励数量
  /// ```
  final int? dataType;
