...
internal class SuitManager: McuManager {
    ...
    /**
     Command allows to get information about roles of manifests supported by the device.
     */
    internal func listManifest(callback: @escaping McuMgrCallback<SuitListResponse>) {
     ...
     */
    internal func getManifestState(for role: McuMgrManifestListResponse.Manifest.Role,
     ...
     */
    internal func cleanup(callback: @escaping McuMgrCallback<McuMgrResponse>) {
     ...
     */
    internal func poll(callback: @escaping McuMgrCallback<McuMgrPollResponse>) {
        send(op: .read, commandId: SuitID.pollImageState, payload: nil, callback: callback)
    }
    
    // MARK: upload(_:delegate:)
    
    internal func upload(_ images: [ImageManager.Image], using configuration: FirmwareUpgradeConfiguration, delegate: SuitManagerDelegate?) {
     ...
     */
    internal func processRecentlyUploadedEnvelope(callback: @escaping McuMgrCallback<McuMgrResponse>) {
        resetUploadVariables()
    ...
    internal func uploadResource(_ resourceData: Data) {
        offset = 0
        pollAttempts = 0
    ...
    internal func cancel(with error: Error? = nil) {
        state = .none
        resetUploadVariables()
...
internal enum SuitManagerError: Error, LocalizedError {
...
internal protocol SuitManagerDelegate: ImageUploadDelegate {
