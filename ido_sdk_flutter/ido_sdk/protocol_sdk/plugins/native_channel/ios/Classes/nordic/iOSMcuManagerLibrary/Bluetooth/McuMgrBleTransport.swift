...
internal class McuMgrBleTransport: NSObject {
    ...
    internal weak var delegate: PeripheralDelegate? {
        didSet {
    ...
    /// concurrently.
    internal var numberOfParallelWrites: Int {
        set {
    ...
    internal var chunkSendDataToMtuSize: Bool = false
    
    internal internal(set) var state: PeripheralState = .disconnected {
        didSet {
            DispatchQueue.main.async {
                self.notifyPeripheralDelegate()
            }
    ...
    internal convenience init(_ peripheral: CBPeripheral, _ configuration: Configuration? = nil) {
        self.init(
    ...
    ///   Protocol (SMP) service.
    /// - parameter uuidConfig: A custom UUID configuration
    internal convenience init(_ targetIdentifier: UUID, _ configuration: Configuration? = nil) {
...
extension McuMgrBleTransport: McuMgrTransport {
    ...
    internal func getScheme() -> McuMgrScheme {
        return .ble
    }
    ...
    internal func connect(_ callback: @escaping ConnectionCallback) {
        callback(.deferred)
    }
    ...
    internal func addObserver(_ observer: ConnectionObserver) {
        observers.append(observer)
    }
    
    internal func removeObserver(_ observer: ConnectionObserver) {
    ...
    private func _send(data: Data, timeoutInSeconds: Int) -> Result<Data, Error> {
        ...
        // Wait for the didUpdateValueFor(characteristic:) to internal the lock.
...
internal enum McuMgrBleTransportConstant {
...
internal enum McuMgrBleTransportError: Error, LocalizedError {
