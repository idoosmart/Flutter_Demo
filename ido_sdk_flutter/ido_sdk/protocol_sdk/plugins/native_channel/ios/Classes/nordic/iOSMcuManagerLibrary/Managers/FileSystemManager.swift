...
internal class FileSystemManager: McuManager {
     ...
     */
    internal static let SMP_SVR_BT_L2CAP_MTU = 495
    ...
    internal func download(name: String, offset: UInt,
    ...
    /// - parameter callback: The callback.
    ///
    internal func upload(name: String, data: Data, offset: UInt,
    ...
    internal func download(name: String, delegate: FileDownloadDelegate?) -> Bool {
    ...
    ///
    /// - returns: True if the upload has started successfully, false otherwise.
    internal func upload(name: String, data: Data,
    ...
    /// - parameter name: The file name.
    /// - parameter callback: The callback.
    internal func status(name: String, callback: @escaping McuMgrCallback<McuMgrFilesystemStatusResponse>) {
        let payload: [String: CBOR] = ["name": CBOR.utf8String(name)]
    ...
    /// - parameter callback: The callback.
    internal func crc32(name: String, offset: UInt64, length: UInt64, callback: @escaping McuMgrCallback<McuMgrFilesystemCrc32Response>) {
    ...
    /// - parameter length: The maximum length of data to read from file to generate checksum.
    /// - parameter callback: The callback.
    internal func sha256(name: String, offset: UInt64, length: UInt64, callback: @escaping McuMgrCallback<McuMgrFilesystemSha256Response>) {
        var payload: [String: CBOR] = [
            "name": CBOR.utf8String(name),
    ...
    /// Close any internal file handles held by `fs_mgmt` upload/download requests that might have stalled or be incomplete.
    ///
    /// - parameter callback: The callback.
    internal func closeAll(name: String, callback: @escaping McuMgrCallback<McuMgrResponse>) {
    ...
    /// Image upload states
    internal enum UploadState: UInt8 {
        case none             = 0
    ...
    internal func cancelTransfer(error: Error? = nil) {
        objc_sync_enter(self)
        if transferState == .none {
    ...
    /// nothing happens.
    internal func continueTransfer() {
        objc_sync_enter(self)
        if transferState == .paused {
    ...
    private lazy var mcuManagerParametersCallback: McuMgrCallback<McuMgrParametersResponse> = { [weak self] response, error in
        ...
        self.storedMcuMgrParametersResponse = response
        // Continue
    ...
    private lazy var uploadCallback: McuMgrCallback<McuMgrFsUploadResponse> = {
        ...
        if let offset = response.off {
            ...
            if self.offset == 0 && offset > 0, let response = self.storedMcuMgrParametersResponse {
                self.processMcuMgrParametersResponse(response)
...
private extension FileSystemManager {
    ...
    func processMcuMgrParametersResponse(_ response: McuMgrParametersResponse) {
        log(msg: "Processing McuMgr Parameters Response.", atLevel: .debug)
        var bufferSize: UInt64! = response.bufferSize
        ...
        if bufferSize > bleTransport.mtu, !bleTransport.chunkSendDataToMtuSize {
            ...
            do {
                log(msg: "SAR Buffer Size of \(bufferSize) is larger than MTU Size of \(transport.mtu). Setting MTU Size to \(bufferSize).", atLevel: .debug)
...
internal enum FileTransferError: Error, LocalizedError {
...
internal enum FileSystemManagerError: UInt64, Error, LocalizedError {
    case noError = 0
    case unknown = 1
    case invalidName = 2
    case notFound = 3
    ...
    internal var errorDescription: String? {
        ...
        case .openFailed:
            return "Error occurred whilst attempting to internal file"
        case .seekFailed:
...
internal protocol FileUploadDelegate: AnyObject {
...
internal protocol FileDownloadDelegate: AnyObject {
