...
internal struct McuMgrSuitEnvelope {
    
    internal let digest: McuMgrSuitDigest
    ...
    internal func image() -> ImageManager.Image? {
    ...
    internal func sizeString() -> String {
...
internal class McuMgrSuitDigest: CBORMappable {
    ...
    internal func hash(for algorithm: Algorithm) -> Data? {
        return modes.first(where: { $0.algorithm == algorithm })?.hash
    }
    
    internal func hashString() -> String {
        var result = ""
        for digest in modes {
...
extension McuMgrSuitDigest {
    ...
    internal enum Algorithm: Int, RawRepresentable, CustomStringConvertible {
        /**
...
internal enum McuMgrSuitParseError: LocalizedError {
