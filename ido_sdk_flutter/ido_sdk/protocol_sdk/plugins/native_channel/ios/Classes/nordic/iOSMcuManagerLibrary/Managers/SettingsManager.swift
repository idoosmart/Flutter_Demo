...
internal class SettingsManager: McuManager {
    ...
    internal func read(name: String, callback: @escaping McuMgrCallback<McuMgrConfigResponse>) {
    ...
    /// - parameter callback: The response callback.
    internal func write(name: String, value: [UInt8], callback: @escaping McuMgrCallback<McuMgrResponse>) {
        let payload: [String:CBOR] = ["name": CBOR.utf8String(name),
    ...
    /// - parameter name: The name of the sys config variable to write.
    /// - parameter callback: The response callback.
    internal func delete(name: String, callback: @escaping McuMgrCallback<McuMgrResponse>) {
        let payload: [String:CBOR] = ["name": CBOR.utf8String(name)]
    ...
    ///
    /// - parameter callback: The response callback.
    internal func commit(callback: @escaping McuMgrCallback<McuMgrResponse>) {
    ...
    ///
    /// - parameter callback: The response callback.
    internal func load(callback: @escaping McuMgrCallback<McuMgrResponse>) {
    ...
    ///
    /// - parameter callback: The response callback.
    internal func save(callback: @escaping McuMgrCallback<McuMgrResponse>) {
...
internal enum SettingsManagerError: UInt64, Error, LocalizedError {
    case noError = 0
    case unknown = 1
    case keyTooLong = 2
    case keyNotFound = 3
